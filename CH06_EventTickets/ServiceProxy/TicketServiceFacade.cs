    ...
    public class TicketServiceFacade
    {
        ...
        public TicketReservationPresentation ReserveTicketsFor(string EventId, int NoOfTkts)
        {
            TicketReservationPresentation reservation =
                                      new TicketReservationPresentation();
            ...
            if (response.Success)
            {
                ...
                reservation.Description = String.Format(
                       "{0}ticket(s) reserved for {1}.<br/>" +
                       "<small> This reservation will expire on {2} at {3}.</small>",
                       response.NoOfTickets, response.EventName,
        ...
        public TicketPresentation PurchaseReservedTicket(
                                                string eventId, string reservationId)
        {
            ...
            if (response.Success)
            {
                ticket.Description = String.Format(
                          "{0}ticket(s) purchased for { 1}.< br />" +
